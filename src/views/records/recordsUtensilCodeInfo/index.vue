<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="动作" prop="utensilCodeAction">
        <el-select v-model="queryParams.utensilCodeAction" placeholder="动作">
          <el-option
            v-for="dict in utensilCodeActionList"
            :key="dict.code"
            :label="dict.desc"
            :value="dict.code+''"
          ></el-option>
    ...
    <el-row :gutter="10" class="mb8">
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['records:recordsUtensilCodeInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['records:recordsUtensilCodeInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['records:recordsUtensilCodeInfo:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['records:recordsUtensilCodeInfo:export']"
        >导出</el-button>
      </el-col>
        <!--<el-col :span="1.5">
            <el-button
    ...
    <el-table v-loading="loading" :data="recordsUtensilCodeInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="剩余数量" align="center" prop="residueNum" />
      <el-table-column label="工艺" align="center" prop="technologyCode" />
      <el-table-column label="工序" align="center" prop="processesName" />
      <el-table-column label="工序" align="center" prop="nextProcessesName" />
      ...
      <el-table-column label="是否最后工序" align="center" prop="finalProcessFlagName" />
      <el-table-column label="动作" align="center" prop="utensilCodeActionName" />
...
<script>
  import { searchItemList, getProcessesList, searchUserListByName, getUtensilCodeActionList } from '@/api/enumsSelect';
import { listRecordsUtensilCodeInfo, getRecordsUtensilCodeInfo, delRecordsUtensilCodeInfo,
    addRecordsUtensilCodeInfo, updateRecordsUtensilCodeInfo } from "@/api/records/recordsUtensilCodeInfo";
import { getToken } from "@/utils/auth";

export default {
  name: "RecordsUtensilCodeInfo",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      upload: {
          ...
          // 上传的地址
          url: process.env.VUE_APP_BASE_API + "/records/recordsUtensilCodeInfo/importData"
      },
      //物料信息
      itemList: [],
  ...
  created() {
    ...
    this.selectUtensilCodeActionList();
    this.getList();
  },
  methods: {
    /** 工序列表 */
    ...
    /** 工序列表 */
    selectUtensilCodeActionList() {
      getUtensilCodeActionList().then(response => {
        this.utensilCodeActionList = response.data;
      });
    },
    /** 查询器具码信息列表 */
    getList() {
      this.loading = true;
      listRecordsUtensilCodeInfo(this.queryParams).then(response => {
        this.recordsUtensilCodeInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getRecordsUtensilCodeInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRecordsUtensilCodeInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            }).finally(()=>this.loading = false);
          } else {
            addRecordsUtensilCodeInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除器具码信息编号为"' + ids + '"的数据项？').then(function() {
        return delRecordsUtensilCodeInfo(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('records/recordsUtensilCodeInfo/export', {
        ...this.queryParams
      }, `recordsUtensilCodeInfo_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
        this.upload.title = "器具码信息导入";
        this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
        this.download('records/recordsUtensilCodeInfo/importTemplate',{...this.queryParams}, '器具码信息导入模板.xlsx');
    },
    // 文件上传中处理
