<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="单号" prop="inboundNumber">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['warehouse:inboundOrderInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['warehouse:inboundOrderInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['warehouse:inboundOrderInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['warehouse:inboundOrderInfo:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="inboundOrderInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="单号" align="center" prop="inboundNumber" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="单号" prop="inboundNumber">
          <el-input v-model="form.inboundNumber" placeholder="请输入单号" />
        </el-form-item>
        <el-form-item label="父图号" prop="parentItemCode">
...
<script>
import { listInboundOrderInfo, getInboundOrderInfo, delInboundOrderInfo, addInboundOrderInfo, updateInboundOrderInfo } from "@/api/warehouse/inboundOrderInfo";

export default {
  name: "InboundOrderInfo",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listInboundOrderInfo(this.queryParams).then(response => {
        this.inboundOrderInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getInboundOrderInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateInboundOrderInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addInboundOrderInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除入库明细编号为"' + ids + '"的数据项？').then(function() {
        return delInboundOrderInfo(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('warehouse/inboundOrderInfo/export', {
        ...this.queryParams
