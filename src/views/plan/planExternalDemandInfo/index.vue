<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['plan:planExternalDemandInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['plan:planExternalDemandInfo:edit']"
        >修改</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['plan:planExternalDemandInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['plan:planExternalDemandInfo:export']"
        >导出</el-button>
      </el-col>
        <!--<el-col :span="1.5">
            <el-button
    ...
    <el-table v-loading="loading" :data="planExternalDemandInfoList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="需求数量" prop="demandQuantity">
          <el-input v-model="form.demandQuantity" placeholder="请输入需求数量" />
        </el-form-item>
        ...
        <el-form-item label="需求配送时间" prop="distributionTime">
        ...
        <el-form-item label="机加工单位" prop="abuttingJointManufacturer">
          <el-input v-model="form.abuttingJointManufacturer" placeholder="请输入机加工单位" />
        </el-form-item>
...
import { listPlanExternalDemandInfo, getPlanExternalDemandInfo, delPlanExternalDemandInfo,
    addPlanExternalDemandInfo, updatePlanExternalDemandInfo } from "@/api/plan/planExternalDemandInfo";
import { getToken } from "@/utils/auth";

export default {
  name: "PlanExternalDemandInfo",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  created() {
    this.queryParams.externalDemandPlanCode = this.$route.params && this.$route.params.planCode;
    this.getList();
  },
  methods: {
    /** 查询需求计划明细列表 */
    getList() {
      this.loading = true;
      listPlanExternalDemandInfo(this.queryParams).then(response => {
        this.planExternalDemandInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getPlanExternalDemandInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePlanExternalDemandInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            }).finally(()=>this.loading = false);
          } else {
            addPlanExternalDemandInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除需求计划明细编号为"' + ids + '"的数据项？').then(function() {
        return delPlanExternalDemandInfo(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('plan/planExternalDemandInfo/export', {
        ...this.queryParams
      }, `planExternalDemandInfo_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
        this.upload.title = "需求计划明细导入";
        this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
        this.download('plan/planExternalDemandInfo/importTemplate',{...this.queryParams}, '需求计划明细导入模板.xlsx');
    },
    // 文件上传中处理
